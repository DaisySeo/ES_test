...
exports.handler = (event, context, callback) => {
    ...
    Promise.all(event.records.map(r => {
        ...
        if (data.messageType === 'CONTROL_MESSAGE') {
            ...
            return Promise.all(promises)
                .then(transformed => {
                    ...
                    const log_data = {
                        start: transformed[0],
                        message: transformed[1],
                        end: transformed[2],
                        report: transformed[3]
                    }
                    
                    if (transformed.length <= 6000000) {
                        success++;
                        return {
                            recordId: r.recordId,
                            result: 'Ok',
                            data: (Buffer.from(JSON.stringify(log_data))).toString('base64'),
                        };
                    } else {
                        failure++;
                        return {
        ...
        if (putRecordBatches.length > 0) {
            ...
            console.log(success, 'records successful, ', failure, ' records failed.');
